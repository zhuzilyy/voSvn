...
public class EaseMessageAdapter extends BaseAdapter{
	...
	private static final int MESSAGE_TYPE_RECV_CARD=14;
	private static final int MESSAGE_TYPE_SENT_CARD=15;
	...
	public int getItemViewType(int position) {
		...
		if (message.getType() == EMMessage.Type.TXT) {
			...
			//自定义名片类型
			if("card".equals(message.getStringAttribute("xzyType",""))){
	...
	protected EaseChatRow createChatRow(Context context, EMMessage message, int position) {
        ...
        case TXT:
            ...
            }else if("card".equals(message.getStringAttribute("xzyType",""))){

				chatRow=new MyChatRowCard(context,message,position,this);
				Log.i("yy","-----------------名片---------------------");
			}else{
