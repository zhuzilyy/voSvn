        ...
        if (type == EMMessage.Type.TXT.ordinal()) {
           /* if(message.getBooleanAttribute(Constant.MESSAGE_ATTR_IS_VIDEO_CALL, false)
            ...
            if (message.getBooleanAttribute(Constant.MESSAGE_ATTR_IS_BIG_EXPRESSION, false)) {
                ...
                //通过名片的扩展字段判断，然后转发
                if ("mingpianType".equals(message.getStringAttribute(MinPianConstant.myExtType, ""))||"ShareType".equals(message.getStringAttribute(ShareConstant.myExtType, ""))) {
                    //xzy:这里选择图片的因为名片只可以删除和转发
                    setContentView(R.layout.em_context_menu_for_image);
                } else {
